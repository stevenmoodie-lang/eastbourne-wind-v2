...
try:
    ...
    for _, day in df_sun.iterrows():
        ...
        for i in range(3):
            ...
            if not d.empty:
                ...
                segments.append({"x_id": f"{day['date']}_{i}", "speed": d['speed'].mean(), "dir": avg_dir, "midday_ref": midday if i == 1 else None})
    ...
    for s in segments:
        ...
        # Mid-day indicator line (very faint)
        if s.get('midday_ref'):
            fig_ribbon.add_vline(x=s['x_id'], line_width=1, line_dash="dash", line_color="rgba(255,255,255,0.15)")
    ...
    for i in range(len(df_hourly)-1):
        ...
        fig_main.add_trace(go.Scatter(x=[p1['time'], p2['time']], y=[p1['speed'], p2['speed']],
            line=dict(color=get_color(p1['speed'], 0.15 if is_night else 1.0), width=2), mode='lines', showlegend=False), row=1, col=1)

    fig_main.add_trace(go.Scatter(x=df_tide['time'], y=df_tide['height'], fill='tozeroy', 
        line=dict(color="rgba(255,255,255,0.3)", width=1), fillcolor="rgba(255,255,255,0.05)", showlegend=False), row=2, col=1)

    # Markers & Subtle Sun Times
    for _, day in df_sun.iterrows():
        # Mid-day line
        ...
        # Sunrise/Sunset Text Labels
        fig_main.add_annotation(x=day['sunrise'], y=max_wind + 8, text=day['sunrise'].strftime('%H:%M'), showarrow=False, font=dict(size=6, color="rgba(255,255,255,0.4)"), row=1, col=1)
        fig_main.add_annotation(x=day['sunset'], y=max_wind + 8, text=day['sunset'].strftime('%H:%M'), showarrow=False, font=dict(size=6, color="rgba(255,255,255,0.4)"), row=1, col=1)
    ...
    fig_main.update_layout(
        ...
        yaxis=dict(visible=False, range=[-5, max_wind + 12]),
        yaxis2=dict(visible=False, range=[0, 2.2])
    )
