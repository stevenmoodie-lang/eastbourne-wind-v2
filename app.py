...
st.markdown("""
    <style>
        ...
        .stPlotlyChart { margin-top: -5px !important; }
    </style>
...
if data and 'hourly' in data:
    ...
    # --- TOP GRAPH: DAYLIGHT ONLY HEATSTRIP ---
    ...
    # Larger Gaps and Labels
    prev_date = None
    for i, row in day_df.iterrows():
        current_date = row['time'].date()
        if prev_date is not None and current_date != prev_date:
            # Increased line_width for a larger visual gap
    ...
    fig_bot = make_subplots(rows=2, cols=1, shared_xaxes=True, vertical_spacing=0.03, row_heights=[0.2, 0.8])
    ...
    fig_bot.update_layout(height=260, margin=dict(t=10, b=0, l=5, r=5), template="plotly_white", hovermode="x unified",
