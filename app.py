...
if data and 'hourly' in data:
    ...
    # --- MAIN GRAPHS ---
    fig_bot = make_subplots(rows=3, cols=1, shared_xaxes=True, vertical_spacing=0.1, row_heights=[0.08, 0.52, 0.40])
    ...
    for i in range(len(sun_data)):
        ...
        for s in range(3):
            ...
            if not df[mask].empty:
                ...
                fig_bot.add_annotation(x=t0+(t1-t0)/2, y=0.5, text="➤", textangle=d_mean-90, showarrow=False, font=dict(size=10, color="white"), row=1, col=1)
    ...
    for d_date in df['date_only'].unique():
        ...
        if not day_block.empty:
            ...
            fig_bot.add_annotation(x=p['time'], y=p['wind'], text=f"<b>{round(p['wind'])}</b>", showarrow=False, yshift=12, xshift=-7, font=dict(size=9, color="white"), row=2, col=1)
            fig_bot.add_annotation(x=p['time'], y=p['wind'], text="➤", textangle=p['dir']-90, showarrow=False, yshift=12, xshift=7, font=dict(size=7, color="white"), row=2, col=1)
            v = day_block.loc[day_block['wind'].idxmin()]
            fig_bot.add_annotation(x=v['time'], y=v['wind'], text=f"<b>{round(v['wind'])}</b>", showarrow=False, yshift=-12, xshift=-7, font=dict(size=9, color="#d1d9e0"), row=2, col=1)
            fig_bot.add_annotation(x=v['time'], y=v['wind'], text="➤", textangle=v['dir']-90, showarrow=False, yshift=-12, xshift=7, font=dict(size=7, color="#d1d9e0"), row=2, col=1)
    ...
    # Center Day Labels in Daylight
    tick_vals = []
    tick_text = []
    for i in range(len(sun_data)):
        mid_day = sun_data.iloc[i]['sunrise'] + (sun_data.iloc[i]['sunset'] - sun_data.iloc[i]['sunrise']) / 2
    ...
    fig_bot.update_layout(
        ...
        xaxis3=dict(showticklabels=False, showgrid=False) # Removes dates from bottom
    )
